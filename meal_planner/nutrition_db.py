...
def load_catalog_entries(settings: AppSettings | None = None) -> list[NutritionEntry]:
    settings = settings or get_settings()
    ...
    if rows is None:
        raise NutritionDatabaseError("SQLite nutrition catalog is empty.")
    return [
        NutritionEntry(
...
def load_target_rows(
    ...
    if rows is None:
        raise NutritionDatabaseError("SQLite nutrition targets are empty.")
